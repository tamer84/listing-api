...
resource "aws_lambda_function" "listing-api" {
  ...
  environment {
    variables = {
      ...
      LOG_LEVEL           = "INFO"
      IDENTITY_TABLE      = data.terraform_remote_state.environment_resources.outputs.identity_table.collect.id
    }
  }
...
resource "aws_api_gateway_stage" "listing-api-live-state" {
  ...
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.listing-api-logs.arn
